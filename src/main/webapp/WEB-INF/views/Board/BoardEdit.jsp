...
<main class="be-main" data-aos="fade-up" data-aos-duration="900">
  <section class="be-card">
    ...
    <form class="be-form"
      ...
      <!-- 이미지 수정 -->
      <div class="be-row">
        <label class="be-label">이미지</label>
        <div class="be-image-preview">
          <c:if test="${not empty dto.b_image}">
            <img src="<c:url value='/images/b_images/${dto.b_image}'/>" alt="게시글 이미지" class="be-image-preview-img" />
          </c:if>
        </div>
        <input type="file" name="b_image" accept="image/*" />
      </div>
