...
<%@ taglib prefix="fmt" uri="jakarta.tags.fmt" %> <%-- ✅ 날짜 포맷 태그 추가 --%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<link rel="stylesheet" href="<c:url value='/css/RecvMessageView.css'/>">
...
<div class="page">
  <div class="glass" data-aos="fade-up" data-aos-duration="850">
    <div class="head">
      <h2 class="title">받은 쪽지 상세보기</h2>
      <div class="actions">
        <a class="btn" href="<c:url value='/Message/RecvMessageList'/>">받은 쪽지함으로</a>
        <a class="btn" href="<c:url value='/Message/MessageWriteForm?recv_nick=${dto.send_nick}'/>">답장하기</a>
        <a class="btn danger" href="<c:url value='/Message/DeleteRecvMessage?ms_no=${dto.ms_no}&type=recv'/>"
    ...
    <table class="msg-table">
      <tr>
        <th>보낸 사람</th>
        <td>
          <a href="<c:url value='/Member/MemberView?m_no=${dto.send_m_no}'/>" class="user-link">
            <div class="user-avatar">
              <img src="<c:url value='/m_images/${dto.send_image}'/>" 
                   onerror="this.onerror=null; this.src='<c:url value='/images/common/default_profile.png'/>';"/>
            </div>
            <span class="user-nick">${dto.send_nick}</span>
          </a>
        </td>
      </tr>
      <tr>
