...
<%@ page import="com.sea_you.dto.NoticeDTO" %>
...
<link rel="stylesheet" href="<c:url value='/css/NoticeDetail.css'/>">
...
<main class="nd-main" data-aos="fade-up" data-aos-duration="900">
  <section class="nd-card">

    <header class="nd-head">
      <h2 class="nd-title"><%= dto.getN_title() %></h2>
      <div class="nd-date">
        작성일: <span><%= (dto.getN_date() != null ? sdf.format(dto.getN_date()) : "") %></span>
      </div>
    </header>

    <article class="nd-body">
      <div class="nd-content"><%= dto.getN_write() %></div>
    </article>

    <div class="nd-actions">
      <a class="btn-ghost" href="<%= request.getContextPath() %>/Notice/NoticeList">목록으로</a>

      <% if(loginMember != null && "ADMIN".equals(loginMember.getM_temp())) { %>
        <a class="btn-primary"
           href="<%= request.getContextPath() %>/Notice/NoticeEdit?n_no=<%= dto.getN_no() %>">수정</a>
