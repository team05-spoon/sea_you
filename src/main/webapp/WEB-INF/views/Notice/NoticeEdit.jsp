...
<link rel="stylesheet" href="<c:url value='/css/NoticeEdit.css'/>">
...
<!-- ✅ 공지 수정 폼(투명 박스) -->
<main class="ne-main" data-aos="fade-up" data-aos-duration="900">
  <section class="ne-card">
    <header class="ne-head">
      <h2 class="ne-title">공지사항 수정</h2>
      <p class="ne-sub">내용을 수정한 뒤 저장을 눌러주세요.</p>
    </header>

    <form class="ne-form" name="NoticeEditForm" action="${pageContext.request.contextPath}/Notice/NoticeEdit" method="post">
      <input type="hidden" name="n_no" value="${dto.n_no}">

      <div class="ne-row">
        <label class="ne-label">공지 제목</label>
        <input class="ne-input" name="n_title" value="${dto.n_title}" required>
      </div>

      <div class="ne-row">
        <label class="ne-label">공지 내용</label>
        <!-- ✅ 기존 dto 내용 나오게 value 채움 -->
        <textarea class="ne-textarea" name="n_write" required>${dto.n_write}</textarea>
      </div>

      <div class="ne-actions">
        <input class="btn-primary" type="submit" value="수정 완료하기">
        <a class="btn-ghost" href="${pageContext.request.contextPath}/Notice/NoticeList">취소하기</a>
      </div>
    </form>
  </section>
</main>
