...
<link rel="stylesheet" href="<c:url value='/css/ReportMemberWriteForm.css'/>">
...
<main class="rm-main" data-aos="fade-up" data-aos-duration="850">
  <section class="rm-card">
    <div class="rm-head">
      <h2 class="rm-title">회원 신고하기</h2>
      <div class="rm-sub">허위 신고는 제재될 수 있습니다.</div>
    </div>

    <form class="rm-form" action="${pageContext.request.contextPath}/Report/ReportMemberWriteForm" method="post">
      <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
      <input type="hidden" name="target_m_no" value="${targetMno}">

      <div class="rm-target">
        신고 대상 번호: <strong>${targetMno}</strong>
      </div>

      <div class="rm-row">
        <label class="rm-label">신고 사유</label>
        <select class="rm-select" name="rm_ctg" required>
      ...
      <div class="rm-row">
        <label class="rm-label">제목</label>
        <input class="rm-input" type="text" name="rm_title" required placeholder="신고 제목을 입력하세요">
      </div>

      <div class="rm-row">
        <label class="rm-label">내용</label>
        <textarea class="rm-textarea" name="rm_content" rows="10" required placeholder="상세 신고 내용을 입력하세요"></textarea>
      </div>

      <div class="rm-btns">
        <button type="submit" class="rm-btn primary">신고 제출</button>
        <button type="button" class="rm-btn ghost" onclick="history.back()">취소</button>
      </div>
    </form>
  </section>
</main>
